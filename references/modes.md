...
4. Agent 도구로 Reviewer 재검토 (이전 맥락을 프롬프트에 포함하거나 SendMessage 활용) → `round2_1_reviewer_review.md`
...
2. 매 라운드 종료 시 합의 판정:
   ...
   - 미합의 → 다음 라운드 (SendMessage로 세션 유지 또는 새 Agent 호출에 맥락 포함)
...
2. 루프: Reviewer 반박 → 패배 감지 → Main 재반박 → 패배 감지 → 반복
   - 각 Reviewer 호출에 SendMessage로 세션 유지 또는 새 Agent 호출에 맥락 포함
