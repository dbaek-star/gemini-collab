...
예: `-m gemini-2.5-pro` 지정 시 → `gemini-2.5-pro` → `gemini-3-flash-preview` → `gemini-2.5-flash` 순서로 시도.
...
- **모델 Fallback**: 선택 모델부터 하위 모델로 4단계 자동 전환 (3-pro → 2.5-pro → 3-flash → 2.5-flash)
